...
if (file_exists($CFG->dirroot . '/local/publicprivate/lib/course.class.php')) {
...
function local_metagroups_sync(progress_trace $trace, $courseid = null) {
    ...
    foreach (array_unique($courseids) as $courseid) {
        ...
        //    continue;
        //}
        $ppclassfound = class_exists('PublicPrivate_Course');
        if (!$ppclassfound && groups_get_course_groupmode($parent) == NOGROUPS) {
            continue;
        }
        // END UCLA MOD: CCLE-3718
        ...
        foreach ($children as $childid) {
            ...
            foreach ($groups as $group) {
                // START UCLA MOD: CCLE-3718 - Group Management for TA sites
                if ($ppcourse && $group->id == $ppcourse->get_group()) {
                    continue;
                }
                // END UCLA MOD: CCLE-3718
